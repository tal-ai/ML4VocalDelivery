...
class Fuse_Learning_Orth(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, waveform_inputs):
        ...
        # before we add fuse the result, we need to calculate the orthognal loss
        orth_0 = torch.diag(torch.matmul(acoustic_encode, semantic_encode.permute(1,0)))
        orth_1 = torch.sqrt(torch.sum(acoustic_encode**2,dim=1))
        orth_2 = torch.sqrt(torch.sum(semantic_encode**2,dim=1))
        orth_loss = torch.sum(torch.abs(orth_0 / (orth_1 * orth_2)))

        fuse_encode = torch.cat([semantic_encode, acoustic_encode],dim=-1)
        logits = self.classifier(fuse_encode)
        return logits, orth_loss
...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_num):
        ...
        for acoustic_inputs, semantic_inputs, label_inputs, _ in train_loader:
            ...
            model.zero_grad()
            prediction, orth_loss = model(input_ids,attention_mask,waveform_inputs)
        ...
        for acoustic_inputs, semantic_inputs, label_inputs, _ in train_loader:
            ...
            prediction, _ = model(input_ids,attention_mask,waveform_inputs)
    ...
    save_path = '/home/work/Projects/GodEye/result/paan+roberta+orth_report.pkl'
